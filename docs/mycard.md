...
画像キャッシュは生 L8 で置き、**表示中の画像 blob だけをオンデマンドで mmap**して
`lv_image` に渡すので、開くときに RAM へロードしない。
...
`factory`(4M, 0x10000–0x410000) 直後の **64KB アライン**位置に 2M（=32×64KB）。名前
検索（subtype は custom `0x40`）:
...
3. `commit()`: magic 入り Header を offset 0 へ write し、ヘッダキャッシュ
   （`s_hdr`）を読み直す。
