...
- **テキスト**: ASCII ランは base-14（本文 `/Helvetica`、名前 `/Helvetica-Bold`）、
  それ以外は**非埋め込み Type0**（`/Encoding /UniJIS-UCS2-H`＋Adobe-Japan1 の CIDFont、
  `FontFile` 無し→ビューア代替。本文 `Ryumin-Light`、名前 `GothicBBB-Medium`）。
  描画文字列は UCS-2BE の hex 文字列 `<....> Tj`。page2 のレイアウト（画像右寄せ＋
  左カラムに名前/URL/メッセージ、折り返し）は `share-layout.ts` が唯一の定義で、
  エディタのプレビューも同モジュールを共有する。
